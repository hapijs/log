...
const register = function (server, options = {}) {
    ...
    if (events.includes('response')) {
        server.events.on('response', (request) => {
            ...
            logger[defaultLevel]('request completed', data, additionalFields);
        });
    }
    ...
    if (events.includes('onRequest')) {
        server.ext('onRequest', (request, h) => {
            ...
            logger[defaultLevel](`${request.method.toUpperCase()} ${request.path} request(${request.info.id}) received`, internals.formatRequestData(request), additionalFields);
            return h.continue;
        });
    }
