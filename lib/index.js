...
const register = function (server, options = {}) {
    ...
    const {
        ...
        ignoreTags = [],
        level = 'info',
        logger = new Stdlogger(process.stdout, process.stderr),
        logLevelMap = {},
        onError = undefined
    } = options;

    internals.validateArrayOfStrings(events, 'events');
    internals.validateArrayOfStrings(ignoreChannels, 'ignoreChannels');
    internals.validateArrayOfStrings(ignorePaths, 'ignorePaths');
    internals.validateArrayOfStrings(ignoreTags, 'ignoreTags');
    internals.validateLogLevel(defaultLevel, 'defaultLevel');
    internals.validateLogLevel(level, 'level');

    Hoek.assert(typeof additionalFields === 'object' && additionalFields !== null, 'additionalFields must be an object');
    Hoek.assert(typeof logger === 'object' && logger !== null, 'logger must be an object');
    ...
    Hoek.assert(typeof onError === 'function' || onError === undefined, 'onError must be a function');
    ...
    if (events.includes('onRequest')) {
        server.ext('onRequest', (request, h) => {
            ...
            logger[defaultLevel](`${request.method.toUpperCase()} ${request.path} request(${request.info.id}) received`, undefined, additionalFields);
            return h.continue;
        });
    }
...
internals.validateArrayOfStrings = function (arr, name) {

    Hoek.assert(Array.isArray(arr), `${name} must be an array`);

    for (let i = 0; i < arr.length; ++i) {
        Hoek.assert(typeof arr[i] === 'string', `${name}[${i}] must be a string`);
    }
};


internals.validateLogLevel = function (level, name) {

    Hoek.assert(typeof level === 'string', `${name} must be a string`);
    Hoek.assert(validLogLevels.has(level), `${level} is not a valid log level`);
};


internals.setupLogLevelMap = function (logLevelMap) {

    Hoek.assert(typeof logLevelMap === 'object' && logLevelMap !== null, 'logLevelMap must be an object');
