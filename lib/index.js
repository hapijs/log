...
const validateLevel = Validate.string().default('info').valid(...Array.from(validLogLevels.keys()));
...
const register = function (server, options = {}) {
    ...
    let maxLogLevel = validLogLevels.get(level);

    server.expose('setLevel', (newLevel) => {

        const validLevel = Validate.attempt(newLevel, validateLevel, 'invalid level');
        maxLogLevel = validLogLevels.get(validLevel);
    });
    ...
    if (events.includes('response')) {
        server.events.on('response', (request) => {
            ...
            const data = {
                req: internals.formatRequestData(request),
                /* $lab:coverage:off$ */
            ...
            logEvent('request completed', data, defaultLevel);
        });
    }
    ...
    if (events.includes('onRequest')) {
        server.ext('onRequest', (request, h) => {
            ...
            logEvent(`${request.method.toUpperCase()} ${request.path} request(${request.info.id}) received`, internals.formatRequestData(request), defaultLevel);
            return h.continue;
        });
    }
...
internals.formatRequestData = function (request) {
